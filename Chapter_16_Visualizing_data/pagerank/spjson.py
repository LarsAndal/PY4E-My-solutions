...
fhand = open("spider.js", "w", encoding="utf-8")  # pylint: disable=R1732
nodes = []
maxrank = None  # pylint: disable=C0103
minrank = None  # pylint: disable=C0103
for row in cur:
    nodes.append(row)
    rank = row[2]
...
for row in cur:
    # print row
    if row[0] not in _map or row[1] not in _map:
        continue
    if count > 0:
        fhand.write(",\n")
    rank = ranks[row[0]]
