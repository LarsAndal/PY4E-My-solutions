...
def fixsender(sender, allsenders=None):
    ...
    if allsenders is not None and sender.endswith("gmane.org"):
        pieces = sender.split("-")
        realsender = None
        for s in allsenders:  # pylint: disable=C0103
        ...
        if realsender is None:
            for s in mapping:  # pylint: disable=C0103
                if s.startswith(pieces[0]):
                    realsender = sender
                    sender = mapping[s]
                    # print(realsender, sender)
...
for message_row in cur_1:
    ...
    cur.execute(
        """INSERT OR IGNORE INTO Messages
         (guid,sender_id,subject_id,sent_at,headers,body) VALUES ( ?,?,?,datetime(?),?,? )""",
        (
            guid,
            sender_id,
            subject_id,
